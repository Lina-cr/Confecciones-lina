...
const OWNER_USERNAME = 'due�a.lina';
      ...
      <div class="product-info">
        ...
        <div class="product-footer">
          ...
          <div class="product-actions">
            <button class="btn-primary btn-add" data-id="${product.id}">Agregar</button>
...
function renderCart() {
  ...
  if (cart.length === 0) {
    cartItems.innerHTML = '<p style="padding: 2rem; text-align: center; color: var(--text-secondary);">Tu carrito está vacío. Agrega productos desde el catálogo.</p>';
  ...
  cart.forEach((item) => {
    ...
    row.innerHTML = `
      ...
      <div class="cart-item-actions">
        ...
        <button class="remove-cart-btn" data-id="${item.id}" type="button">Eliminar</button>
      </div>
    `;
...
function renderRegisteredUsers() {
  ...
  if (users.length === 0) {
    registeredUsersContainer.innerHTML = '<p>No hay usuarios registrados a�n.</p>';
    return;
  }
...
function updateOwnerPreview() {
  ...
  const category = productCategoryInput?.value.trim() || 'Categor�a';
...
function setAuthMode(mode) {
  ...
  if (mode === 'login') {
    ...
    authInfo.textContent = 'Ingresa tu usuario y contrase�a para entrar.';
    ...
    authInfo.textContent = 'Crea tu cuenta con usuario y contrase�a.';
...
function updateAuthUI() {
  ...
  if (!loginBtn) return;
  if (currentUser) {
    loginBtn.textContent = `Cerrar sesi�n (${currentUser})`;
    loginBtn.setAttribute('aria-label', `Cerrar sesi�n de ${currentUser}`);
  } else {
    loginBtn.textContent = 'Iniciar sesi�n';
    loginBtn.setAttribute('aria-label', 'Iniciar sesi�n');
  }
}
...
function handleUserLogout() {
  ...
  renderCart();
  updateAuthUI();
  alert('Has cerrado sesi�n. Si deseas iniciar sesi�n de nuevo, presiona Iniciar sesi�n.');
}
...
function handleCheckout() {
  // Allow checkout without authentication (guests allowed)
  if (cart.length === 0) {
    alert('Tu carrito est� vac�o. Agrega productos antes de encargar.');
    return;
  }

  const { username: currentUser, lines, total } = buildOrderSummary();
  const summaryText = lines.map((line) => `� ${line}`).join('\n');
  const message = `??? *Pedido Confecciones Lina*\n\n*Cliente:* ${currentUser}\n\n*Productos:*\n${summaryText}\n\n*Total:* ${formatPrice(total)}\n\n*Direcci�n: tienda Casa color esmeralda, 150 metros este de la plaza de deportes, a mano izquierda en Portal�n.*\n\n*El pedido es a nombre de Confecciones Lina.*\n\nGracias por tu pedido ??`;
  const confirmationText = `Resumen de tu encargo:\n\nCliente: ${currentUser}\n\nProductos:\n${summaryText}\n\nTotal: ${formatPrice(total)}\n\nDirecci�n: tienda Casa color esmeralda, 150 metros este de la plaza de deportes, a mano izquierda en Portal�n.\n\n�Deseas enviar este encargo por WhatsApp?`;
  const confirmation = window.confirm(confirmationText);
  if (!confirmation) return;

  const waUrl = `https://wa.me/52162564963?text=${encodeURIComponent(message)}`;
...
if (productForm) {
  ...
  productForm.addEventListener('submit', (event) => {
    ...
    const newProduct = {
      ...
      category: productCategoryInput?.value.trim() || 'Sin categor�a',
      image:
        selectedImage ||
  ...
  loginForm.addEventListener('submit', (event) => {
    ...
    if (authMode === 'register') {
      if (!username || !password) {
        if (loginError) loginError.textContent = 'Ingresa usuario y contrase�a para registrarte.';
        return;
      }
      if (username === OWNER_USERNAME) {
        if (loginError) loginError.textContent = 'Ese nombre de usuario no est� disponible.';
        return;
      }
      const registeredUsers = loadRegisteredUsers();
      const existing = registeredUsers.find((user) => user.username === username);
      if (existing) {
        if (loginError) loginError.textContent = 'Ese usuario ya est� registrado.';
        return;
      }
      ...
      if (loginError) loginError.textContent = '';
      closeLoginModal();
      alert(`Cuenta creada y has ingresado como ${username}. Tu carrito est� guardado.`);
      return;
    }

    if (username === OWNER_USERNAME) {
      if (!password) {
        if (loginError) loginError.textContent = 'Ingresa la contrase�a de la due�a.';
        return;
      }
      ...
      if (loginError) loginError.textContent = 'Usuario o contrase�a incorrectos.';
      return;
    }
    ...
    const existing = registeredUsers.find((user) => user.username === username);
    if (!existing) {
      if (loginError) loginError.textContent = 'Usuario no encontrado. Reg�strate primero.';
      return;
    }
    if (!password) {
      if (loginError) loginError.textContent = 'Ingresa tu contrase�a.';
      return;
    }
    ...
    if (loginError) loginError.textContent = '';
    closeLoginModal();
    alert(`Bienvenido, ${username}! Tu carrito est� guardado en tu cuenta.`);
  });
}
